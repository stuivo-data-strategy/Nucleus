...
class WorkflowService {
    ...
    async resolveDirectManager(personId, initiatorName) {
    ...
    async resolveCostCentreOwner(personId, existingApproverIds, initiatorName) {
    ...
    `, { personId });
        ...
        if (currentCcId.toString)
            currentCcId = currentCcId.toString();
        let baseCcName = '';
        let currentPath = '';
        while (currentCcId) {
    ...
    `, { rId: `role:${roleBaseName}` });
        const rArr = roleRes[0];
        if (rArr && rArr[0] && rArr[0].holders && rArr[0].holders.length > 0) {
            // Pick first active holder for PoC
            const holder = rArr[0].holders.find((h) => h.status === 'active') || rArr[0].holders[0];
    ...
    async resolveApprovalChain(templateName, initiatorId, context) {
    ...
    async createInstance(templateName, initiatorId, subjectType, subjectId, context) {
        // Graph resolution
        const { steps: resolvedSteps, resolution_log, skipped_steps } = await this.resolveApprovalChain(templateName, initiatorId, context);
        const instanceSteps = resolvedSteps.map((approver, index) => ({
            order: index + 1,
    ...
    async processAction(instanceId, actorId, action, note) {
        ...
        if (action === 'respond') {
            ...
            if (action === 'approve') {
                ...
                if (!note)
                    throw new Error('Query requires a note');
                step.status = 'queried';
                step.acted_at = new Date().toISOString();
                step.note = note;
    ...
    async getWorkflowTimeline(instanceId) {
        const res = await this.db.query(`SELECT * FROM type::record($id)`, { id: instanceId });
        const instanceArray = res[0];
