...
class ExpenseService {
    ...
    constructor(db) {
        this.db = db;
        this.policySvc = new policy_service_1.PolicyService(db);
        this.workflowSvc = new workflow_service_1.WorkflowService(db);
    }
    // ─── Helpers ───────────────────────────────────────────────────────────────
    initials(p) {
    ...
    async getClaimsForClaimant(claimantId) {
        try {
            const res = await this.db.query(`SELECT * FROM expense_claim WHERE claimant = type::record($id) ORDER BY created_at DESC`, { id: claimantId });
            const raws = res[0];
    ...
    async getClaimsForApprover(approverId) {
    ...
    // ─── POST /expenses ─────────────────────────────────────────────────────────
    async submitClaim(claimantId, body) {
        const { category, date, has_receipt, description, currency = 'GBP', receipt_url, receipt_amount, claim_amount, partial_claim, partial_reason, exception_requested, exception_justification, } = body;
        ...
        const claimData = {
            reference,
            claimant: new surrealdb_1.StringRecordId(claimantId),
            category,
            amount: claimAmount,
            currency,
            description,
        ...
        const workflow = await this.workflowSvc.createInstance(workflowTemplate, claimantId, 'expense_claim', claimId, { amount: claimAmount, category });
        ...
        await this.db.query(`UPDATE type::record($id) MERGE { workflow_instance: type::record($wi), status: $status }`, { id: claimId, wi: workflowId, status: pendingStatus });
    ...
    async processAction(claimId, actorId, action, note) {
        // 1. Fetch the claim
        ...
        if (action === 'approve') {
            ...
            if (isFinalStep) {
                ...
                await this.db.query(`UPDATE type::record($id) MERGE { policy_result_approval: $pr }`, { id: claimId, pr: approvalPolicyResult });
            }
        }
    ...
    async exportApproved(actorId) {
        // Permission check — must hold finance_approver role
        const roleCheck = await this.db.query(`SELECT <-has_role<-person[WHERE id = type::record($personId)].id AS holders FROM role:finance_approver`, { personId: actorId });
        const rArr = roleCheck[0];
        ...
        const claimsRes = await this.db.query(`SELECT * FROM expense_claim WHERE status = 'approved' ORDER BY created_at DESC`);
        const claims = claimsRes[0];
        ...
        const allRules = rulesRes[0];
        const ruleByCategory = {};
        for (const r of allRules)
            ruleByCategory[r.category] = r;
        const rows = [];
        let totalAmount = 0;
        for (const claim of claims) {
            ...
            if (claim.workflow_instance) {
                try {
                    ...
                    const approveAction = timeline.actions.slice().reverse().find((a) => a.action === 'approve');
                    if (approveAction) {
    ...
    async getApprovedForFinance(actorId) {
        // Role check
        const roleCheck = await this.db.query(`SELECT <-has_role<-person[WHERE id = type::record($personId)].id AS holders FROM role:finance_approver`, { personId: actorId });
        const rArr = roleCheck[0];
        const hasRole = rArr && rArr[0] && rArr[0].holders && rArr[0].holders.length > 0;
        ...
        const claimsRes = await this.db.query(`SELECT * FROM expense_claim WHERE status IN ['approved', 'posted'] ORDER BY created_at DESC`);
        const claims = claimsRes[0];
        if (!claims || claims.length === 0)
            return { approved: [], posted: [], total_pending: 0 };
        const rulesRes = await this.db.query(`SELECT * FROM policy_rule`);
        const allRules = rulesRes[0];
        const ruleByCategory = {};
        for (const r of allRules)
            ruleByCategory[r.category] = r;
        const enrichRow = async (claim) => {
            const claimId = claim.id?.toString ? claim.id.toString() : claim.id;
            const claimantId = claim.claimant?.toString ? claim.claimant.toString() : claim.claimant;
            const person = await this.fetchPerson(claimantId);
            const rule = ruleByCategory[claim.category] || {};
    ...
    async markPosted(claimIds) {
        let updated = 0;
        for (const id of claimIds) {
            try {
                await this.db.query(`UPDATE type::record($id) MERGE { status: 'posted' } WHERE status = 'approved'`, { id });
                updated++;
            }
