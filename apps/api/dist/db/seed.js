...
async function seed() {
    ...
    const db = connection_1.dbConnection.db;
    ...
    for (const o of orgs)
        await db.query(`UPSERT ${o.id} MERGE $data`, { data: cleanData(o) });
    ...
    const addPerson = async (id, f, l, title, job_c, org_u, cc, manager) => {
        ...
        // Edges
        if (manager)
            edgeReports.push({ in: id, out: manager, type: 'direct' });
    ...
    for (const p of positions)
        await db.query(`UPSERT ${p.id} MERGE $data`, { data: cleanData(p) });
    ...
    for (const wf of wfs)
        await db.query(`UPSERT ${wf.id} MERGE $data`, { data: cleanData(wf) });
    ...
    for (const br of brules)
        await db.query(`UPSERT ${br.id} MERGE $data`, { data: cleanData(br) });
    ...
    for (const r of roles)
        await db.query(`UPSERT ${r.id} MERGE $data`, { data: cleanData(r) });
    // Grant roles
