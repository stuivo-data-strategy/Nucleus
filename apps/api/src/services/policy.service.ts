...
import { PolicyCheck, PolicyResult, PolicyAuditEntry, PolicyRule } from '@nucleus/types';
...
export class PolicyService {
  ...
  async validateClaim(claim: {
    ...
    try {
      ...
      const dupQuery = `
        ...
        AND status != 'rejected'
      `;
      const dupRes = await this.db.query(dupQuery, { claimant: new StringRecordId(claim.claimant_id), cat: claim.category, low: lowerBound, high: upperBound });
      const dupArr = dupRes[0] as any[];
  ...
  async logPolicyAudit(
    claimId: string,
    evaluationPoint: 'capture' | 'submission' | 'approval_review' | 'admin_change',
    result: any,
    evaluatedBy: string
  ): Promise<void> {
    const data = {
