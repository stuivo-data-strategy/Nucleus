        ...
        <>
          ...
          >
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
               <h2 className="text-lg font-bold text-[#1B2A4A] flex items-center gap-3">
                 Notifications
               ...
               <div className="flex items-center gap-2">
                 <button className="text-xs font-semibold text-[#2E8B8B] hover:text-[#1B2A4A] transition-colors p-2">Mark all read</button>
            ...
            <div className="flex gap-4 px-6 border-b border-gray-100">
               <button className="text-[#2E8B8B] font-bold text-xs uppercase tracking-wider py-3 border-b-2 border-[#2E8B8B]">All</button>
            ...
            <div className="flex-1 overflow-y-auto bg-gray-50 p-2 space-y-2">
               {(!notifications || notifications.length === 0) ? (
                 ...
                 <div key={n.id} className={`p-4 rounded-xl border ${n.read ? 'bg-white border-gray-100' : 'bg-white border-l-4 border-l-[#2E8B8B] border-t-transparent border-r-transparent border-b-transparent'} shadow-sm relative group`}>
                    {!n.read && <div className="absolute top-4 right-4 w-2 h-2 rounded-full bg-[#2E8B8B]"></div>}
                    <h4 className="font-bold text-[#1B2A4A] text-sm mb-1 pr-6">{n.title}</h4>
                    <p className="text-xs text-gray-500 mb-3">{n.body}</p>
                    {n.actions && n.actions.map((act:any) => (
                       <button key={act.action} className="mr-2 px-3 py-1.5 bg-white border border-[#2E8B8B] text-[#2E8B8B] text-xs font-bold rounded-md hover:bg-teal-50 transition-colors">{act.label}</button>
                    ))}
               ...
               <div className="p-4 rounded-xl bg-white border-l-4 border-l-purple-500 border-t-transparent border-r-transparent border-b-transparent shadow-sm relative group">
                    ...
                    <div className="flex items-center gap-2 mb-1">
                       ...
                       <h4 className="font-bold text-[#1B2A4A] text-sm">Query Requested</h4>
                    </div>
