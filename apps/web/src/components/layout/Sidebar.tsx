  ...
  return (
    <div className="w-64 bg-white border-r border-[#E8ECF2] h-screen flex flex-col hidden lg:flex shrink-0">
      <div className="h-16 flex items-center px-6 border-b border-[#E8ECF2]">
         <div className="w-8 h-8 rounded-full bg-[#000053] text-[#6cffc6] flex items-center justify-center font-bold mr-3 text-sm">N</div>
         <span className="font-display font-bold text-[#000053] text-xl tracking-tight">Nucleus</span>
      </div>
        ...
        {nav.map((section, i) => (
          <div key={i} className="mb-6 pb-2">
            ...
            {section.items.map(item => {
              ...
              return (
                <Link key={item.name} href={item.href} className={`flex items-center justify-between px-2 py-2 rounded-lg transition-colors mb-1 ${active ? 'bg-[#e8fff5] text-[#000053] font-bold border-l-[3px] border-[#6cffc6]' : 'text-gray-500 hover:bg-[#f5f5fc] hover:text-[#000053] font-medium'}`}>
                   <span>{item.name}</span>
      ...
      {user && (
        <div className="p-4 border-t border-[#E8ECF2] cursor-pointer hover:bg-gray-50 flex items-center gap-3 transition-colors m-2 rounded-lg">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-bold text-[#000053] truncate">{user.name}</p>
