...
const expenseRoutes = async (fastify) => {
    ...
    function srv() {
        return new expense_service_1.ExpenseService((0, connection_1.getDb)());
    }
    // ─── GET /expenses ─────────────────────────────────────────────────────────
    // Query param ?role=claimant (default) | approver
    fastify.get('/', async (request, reply) => {
        const role = request.query.role || 'claimant';
        const actor = actorId(request);
        try {
            const service = srv();
            const claims = role === 'approver'
    ...
    fastify.post('/preview-route', async (request, reply) => {
        const actor = actorId(request);
    ...
    fastify.post('/calculate-distance', async (request, reply) => {
    ...
    fastify.get('/mileage-profile', async (request, reply) => {
        const actor = actorId(request);
        try {
    ...
    fastify.post('/mark-posted', async (request, reply) => {
        const body = request.body;
